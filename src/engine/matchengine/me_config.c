...
int add_market(json_t *row) {
    for (size_t i = 0; i < settings.market_num; ++i) {        
        if (strcasecmp(settings.markets[i].name, json_string_value(json_object_get(row, "name")))==0) {
            int idx=i;

            read_cfg_int(row, "fee_prec", &settings.markets[idx].fee_prec, false, 4);
            read_cfg_mpd(row, "min_amount", &settings.markets[idx].min_amount, "0.01");
            ...
            read_cfg_str(stock, "name", &settings.markets[idx].stock, NULL);
            read_cfg_int(stock, "prec", &settings.markets[idx].stock_prec, true, 0);
            ...
            read_cfg_str(money, "name", &settings.markets[idx].money, NULL);
            read_cfg_int(money, "prec", &settings.markets[idx].money_prec, true, 0);

            printf("[info] updated market: %s\n", settings.markets[i].name);
            return 0;
        }
    }

    printf("[info] didnt find market: %s\n", json_string_value(json_object_get(row, "name")));

    int total=settings.market_num+1;
    settings.markets=realloc(settings.markets, sizeof(struct market) * total);

    // if (mkt) {
        int idx=settings.market_num;
        // settings.markets=mkt;

        read_cfg_str(row, "name", &settings.markets[idx].name, NULL);
        read_cfg_int(row, "fee_prec", &settings.markets[idx].fee_prec, false, 4);
        read_cfg_mpd(row, "min_amount", &settings.markets[idx].min_amount, "0.01");
        ...
        settings.market_num++;
        printf("[info] created market: %s\n", settings.markets[idx].name);

        if (!register_market(&settings.markets[idx])) {
            return -__LINE__;
        }
        return 0;
}
