...
static int execute_limit_ask_order(bool real, market_t *m, order_t *taker)
{
    ...
    while ((node = skiplist_next(iter)) != NULL) {
        // if there's no more left just ignore
        if (mpd_cmp(taker->left, mpd_zero, &mpd_ctx) == 0) {
            break;
        }

        order_t *maker = node->value;
        // taker is selling, maker is buying, so price of taker must be
        // inferior or equal to price of maker
        if (mpd_cmp(taker->price, maker->price, &mpd_ctx)>0) {
            break;
        }

        // price must be of maker, he added liquidity,
        // he gets to set his price
        mpd_copy(price, maker->price, &mpd_ctx);

        // get amount that maker is able to deliver
        ...
        // XXX
        // ---- subtract the trade amount from the seller
        ...
        // XXX: should be money-ask_fee
        // ++++ add the trade value to the seller money balance
        ...
        // XXX: should be deal+bid_fee
        // ---- subtract the trade value from the buyer money
        balance_sub(maker->user_id, BALANCE_TYPE_FREEZE, m->money, deal);
        if (real) {
            append_balance_trade_sub(maker, m->money, deal, price, amount);
        }
        // ++++ add the trade amount to the buyer stock balance
        ...
        // XXX: should be money instead of stock
        if (mpd_cmp(bid_fee, mpd_zero, &mpd_ctx) > 0) {
            // balance_sub(maker->user_id, BALANCE_TYPE_AVAILABLE, m->amount, bid_fee);
            balance_sub(maker->user_id, BALANCE_TYPE_AVAILABLE, m->money, bid_fee);
            if (real) {
...
static int execute_limit_bid_order(bool real, market_t *m, order_t *taker)
{
    ...
    while ((node = skiplist_next(iter)) != NULL) {
        ...
        if (mpd_cmp(bid_fee, mpd_zero, &mpd_ctx) > 0) {
            balance_sub(taker->user_id, BALANCE_TYPE_AVAILABLE, m->money, bid_fee);
            if (real) {
...
// limit orders pay the maker_fee
int market_put_limit_order(bool real, json_t **result, market_t *m, uint32_t user_id, uint32_t side, mpd_t *amount, mpd_t *price, mpd_t *taker_fee, mpd_t *maker_fee, const char *source)
{
    ...
    if (side == MARKET_ORDER_SIDE_ASK) {
        ...
        mpd_t *fees = mpd_new(&mpd_ctx);
        mpd_t *require = mpd_new(&mpd_ctx);

        mpd_mul(require, amount, price, &mpd_ctx);
        mpd_mul(fees, require, maker_fee, &mpd_ctx);
        mpd_add(require, require, fees, &mpd_ctx);
...
// market orders pay the taker_fee
int market_put_market_order(bool real, json_t **result, market_t *m, uint32_t user_id, uint32_t side, mpd_t *amount, mpd_t *taker_fee, const char *source)
{
    if (side == MARKET_ORDER_SIDE_ASK) {
        ...
        // no bids for sell order, fail immediately
        if (node == NULL) {
            skiplist_release_iterator(iter);
            return -3;
        }
        ...
        skiplist_iter *iter = skiplist_get_iterator(m->asks);
        skiplist_node *node = skiplist_next(iter);
        // no bids for buy order, fail immediately
        if (node == NULL) {
            skiplist_release_iterator(iter);
            return -3;
        }
        ...
        mpd_mul(require, order->price, amount, &mpd_ctx);
        mpd_mul(fees, require, taker_fee, &mpd_ctx);
        mpd_add(require, require, fees, &mpd_ctx);
