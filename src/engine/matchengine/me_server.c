...
static int on_cmd_order_query(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    market_t *market = get_market(market_name);
    if (market == NULL) {
        printf("could not find market %s\n", market_name);
        return reply_error_invalid_argument(ses, pkg);
    }
    ...
    if (limit > ORDER_LIST_MAX_LEN) {
        printf("limit is too big limit=%d, ORDER_LIST_MAX_LEN=%d\n", limit, ORDER_LIST_MAX_LEN);
        return reply_error_invalid_argument(ses, pkg);
    }
...
/**
  * @author Ayrton Gomes <com.ayrton@gmail.com>
  * @date 2019-04-03 18:58
*/
static int on_cmd_market_create(nw_ses *ses, rpc_pkg *pkg, json_t *params) {
    if (json_array_size(params) != 1)
        return reply_error_invalid_argument(ses, pkg);

    json_t *obj=json_array_get(params, 0);
    if (!json_is_object(obj))
        return reply_error_invalid_argument(ses, pkg);

    if (!json_is_string(json_object_get(obj, "name")))
        return reply_error_invalid_argument(ses, pkg);

    json_t *money=json_object_get(obj, "money");
    if (!money || !json_is_object(money)) {
        return reply_error_invalid_argument(ses, pkg);
    }
    if (!json_is_string(json_object_get(money, "name")))
        return reply_error_invalid_argument(ses, pkg);
    if (!json_is_integer(json_object_get(money, "prec")))
        return reply_error_invalid_argument(ses, pkg);


    json_t *stock=json_object_get(obj, "stock");
    if (!stock || !json_is_object(stock)) {
        return reply_error_invalid_argument(ses, pkg);
    }
    if (!json_is_string(json_object_get(stock, "name")))
        return reply_error_invalid_argument(ses, pkg);
    if (!json_is_integer(json_object_get(stock, "prec")))
        return reply_error_invalid_argument(ses, pkg);

    if (!json_is_string(json_object_get(obj, "min_amount")))
        return reply_error_invalid_argument(ses, pkg);

    int ret;
    int added=add_market(obj);
    if (!added) {
        // TODO
        append_operlog("add_market", obj);
        // json_object_set_new(result, "market", "OK");
        json_t *result = json_object();
        ret = reply_result(ses, pkg, result);
        json_decref(result);
        // ret = reply_result(ses, pkg, obj);

    } else {
        // ret=added;
        ret=reply_error_internal_error(ses, pkg);
    }

    return ret;
}

static int on_cmd_asset_create(nw_ses *ses, rpc_pkg *pkg, json_t *params) {
    if (json_array_size(params) != 1)
        return reply_error_invalid_argument(ses, pkg);
    ...
    if (!json_is_string(json_object_get(obj, "name")))
        return reply_error_invalid_argument(ses, pkg);
    if (!json_is_integer(json_object_get(obj, "prec_save")))
        return reply_error_invalid_argument(ses, pkg);
    if (!json_is_integer(json_object_get(obj, "prec_show")))
        return reply_error_invalid_argument(ses, pkg);

    int ret;
    ret=asset_create(json_string_value(json_object_get(obj, "name")), 
                     json_integer_value(json_object_get(obj, "prec_save")),
                     json_integer_value(json_object_get(obj, "prec_save")));

    if (ret==0) {
        json_t *result = json_object();
        json_object_set(result, "asset", obj);
        append_operlog("asset.create", obj);
        ret = reply_result(ses, pkg, result);
        json_decref(result);
        return ret;
