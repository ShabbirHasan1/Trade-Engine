...
int load_balance(MYSQL *conn, const char *table)
{
    ...
    while (true) {
        ...
        for (size_t i = 0; i < num_rows; ++i) {
            ...
            printf("load_balance(); user_id=%d asset=%s balance=%s\n", user_id, asset, row[4]);
...
static int load_oper(json_t *detail)
{
    ...
    if ((params == NULL || !json_is_array(params)) && (strcmp(method, "add_market")!=0) && 
            (strcmp(method, "asset.create")!=0))
        return -__LINE__;
    ...
    } else if (strcmp(method, "add_market") == 0) {
        ret = add_market(params);
        // ret = 1;

    } else if (strcmp(method, "asset.create") == 0) {
        ret=asset_create(json_string_value(json_object_get(params, "name")), 
             json_integer_value(json_object_get(params, "prec_save")),
             json_integer_value(json_object_get(params, "prec_save")));
