...
static int load_market_deals(redisContext *context, sds key, struct market_info *info)
{
    ...
    if (reply == NULL) {
        log_info("couldnt fetch reply from redis.");
        return -__LINE__;
    } else {
        log_info("fetched reply from redis. Total %d elements", (int)reply->elements);
    }
...
static int init_market()
{
    ...
    for (size_t i=0; i<len; ++i) {
        ...
        if (info == NULL) {
            ...
            log_stderr("create market %s", name);
        }
...
{
    log_info("flush_market() disabled");
...
{
    log_info("on_redis_timer()");
    int pid = fork();
    printf("on_redis_timer %d\n", pid);
    if (pid == 0) {
        _exit(clear_redis());
    }
}
...
static int64_t get_message_offset(void)
{
    ...
    printf("redisCommand(GET k:offset)\n");
    redisReply *reply = redisCommand(context, "GET k:offset");
    printf("=== result: === \n");
...
int init_message(void)
{
    ...
    printf("attempt to connect to redis [init_message:%d]\n", __LINE__);
    // redisContext *context = redis_sentinel_connect_master(redis);

    // if (context == NULL) {
    //     printf("could not connect to redis [init_message:%d]\n", __LINE__);
    //     return -__LINE__;
    // }

    // printf("connected to redis [init_message]\n");
    ret = init_market();

    if (ret < 0) {
        printf("--- init_market error [init_message] ---\n");        
        return ret;
    }

    // printf("get_message_offset() [try]\n");
