...
static int on_cmd_market_create(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    if (market_exist(market)) {
        // return reply_error_invalid_argument(ses, pkg);
        result = json_integer(301);

    } else {
        // convert json
        struct market_info *info = create_market(market);
        if (!info) {
            return reply_error_internal_error(ses, pkg);
        }

        result = json_integer(200);
    }
...
static void svr_on_recv_pkg(nw_ses *ses, rpc_pkg *pkg)
{
    ...
    case CMD_MARKET_CREATE:
        log_debug("from: %s cmd market create, sequence: %u params: %s", nw_sock_human_addr(&ses->peer_addr), pkg->sequence, params_str);
