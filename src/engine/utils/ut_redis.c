...
{
    printf("SENTINEL get-master-addr-by-name");
    redis_sentinel_node *curr = context->list;
...
{
    redis_addr addr = {"127.0.0.1", 6379};
    printf("try connect to redis-sentinel [timeout]\n");
    struct timeval timeout = { 3, 0 };

    // redisContext *redis = redisConnect(addr.host, addr.port);
    redisContext *redis = redisConnectWithTimeout(addr.host, addr.port, timeout);

    if (redis == NULL || redis->err) {
        if (redis) {
            if (redis->err) {
                printf("cant connect to redis %d\n", redis->err);
            }
            redisFree(redis);
        }
        return NULL;
